...
def read_logs(log_path: str, filename: str) -> json:
    assert os.path.exists(os.path.join(log_path, filename)), "Filename {} not found".format(os.path.join(log_path, filename))
    with open(os.path.join(log_path, filename), "r+", encoding="utf-8") as f:
        return json.load(f)


def get_training_logs_path(folder: str, algo: str, env_id: str, exp_id: int, resume_dir: str = None) -> str:
...
def parse_experiment_file(
    ...
    assert env_name in ENV_NAMES, "Env name {} not valid. Choose among: {}".format(env_name, ENV_NAMES)
    result = []
    f = False
    num_experiments = 0
    ...
    with open(exp_file, "r+", encoding="utf-8") as f:
        for line in f.readlines():
            ...
            if return_failures and "INFO:experiments:FAIL -" in line:
                env_config_str = line.split(" - ")[1].split(":")[0].replace("Failure probability for env config ", "")
                result.append(get_env_config_of_env(env_config_str=env_config_str, env_name=env_name))
            elif not return_failures and f is True and "Failure probability for env config" in line:
                env_config_str = line.split(":")[2].replace("FAIL - ", "").replace("Failure probability for env config ", "")
                failure_probability = float(line.split(":")[3].replace(" ", "").split("(")[1].split(",")[0])
                env_config = get_env_config_of_env(env_config_str=env_config_str, env_name=env_name)
                result.append((env_config, failure_probability))
            elif not return_failures and "DEBUG:experiments:Num experiments:" in line:
                current_episodes_count = int(line.replace(" ", "").split(":")[3].split("/")[0])
                total_episodes_count = int(line.replace(" ", "").split(":")[3].split("/")[1])
