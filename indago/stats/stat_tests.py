...
def fisher_exact_2(list_a: List[float], list_b: List[float]) -> Tuple[float, float]:
    ...
    contingency_table = np.asarray([[num_failures_a, num_failures_b], [num_non_failures_a, num_non_failures_b]])
...
def fisher_exact(*lists, adjust: bool = True) -> List[Tuple[float, float]]:
    ...
    if adjust:
        reject, p_values_corrected, corrected_alpha_sidak, corrected_alpha_bonferroni = multipletests(pvals=p_values)
        return list(zip(odds_ratios, [round(p_value, 4) for p_value in p_values_corrected]))
...
def anova_plus_tukey(lists: List[List], groups: List[str], alpha: float = 0.05) -> Optional[Union[TukeyHSDResults]]:
    # test normality and other assumptions of one-way Anova and Tukey first
    f_statistic, p_value = stats.f_oneway(*lists)
    if p_value <= alpha:
        # create DataFrame to hold data
        df = pd.DataFrame({"score": [it for lst in lists for it in lst], "group": np.repeat(groups, repeats=len(lists[0]))})
