...
def cluster_data(data: np.ndarray, n_clusters_interval: Tuple, seed: int, silhouette_threshold: int,) -> Tuple:
    ...
    if n_clusters_interval[0] != 1:
        ...
        for n_clusters in range_n_clusters:
            ...
            try:
                silhouette_avg = silhouette_score(data, cluster_labels, random_state=seed)
                ...
                if silhouette_avg > optimal_score:
                    if silhouette_threshold < 0 or optimal_score == -1:
                        ...
                        logger.debug("New optimal silhouette score: {}. Num clusters: {}".format(silhouette_avg, n_clusters))
                    else:
                        assert (
                            0 < silhouette_threshold <= 100
                        ), "Silhouette threshold needs to be in (0, 100]. Found {}".format(silhouette_threshold)
                        percentage_increase = round(100 * (silhouette_avg - optimal_score) / optimal_score, 2)
                        if percentage_increase >= silhouette_threshold:
                            optimal_score = silhouette_avg
                            optimal_n_clusters = n_clusters
                            logger.debug(
                                "New optimal silhouette score: {} > {}% of previous score {}. Num clusters: {}".format(
                                    silhouette_avg, silhouette_threshold, percentage_increase, n_clusters
                                )
                            )
            except ValueError:
