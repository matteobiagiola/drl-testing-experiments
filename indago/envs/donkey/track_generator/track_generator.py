...
from indago.envs.donkey.track_generator.unity.utils import compute_direction, get_track_points, has_loops, track_closed
...
class TrackGenerator:
    ...
    def generate(self) -> List[TrackElem]:
        ...
        result = [TrackElem(command=Command.S, value=randomness.get_random_int(low=self.min_value, high=self.max_value))]
        for i in range(self.length - 2):
            if result[-1].command.name == Command.DY.name:
                # L or R command after DY
                track_elem = TrackElem(
                    command=randomness.get_random_command(excluded_commands=[Command.DY, Command.S]),
                    value=randomness.get_random_int(low=self.min_value, high=self.max_value),
                )
            elif (
                ...
                # S or DY command
                command = randomness.get_random_command(excluded_commands=[Command.R, Command.L])
                if command.name == Command.DY.name:
                    value = round(randomness.get_random_float(low=self.min_value, high=MAX_DY_VALUE), 1)
                else:
                    value = randomness.get_random_int(low=self.min_value, high=self.max_value)
                track_elem = TrackElem(command=command, value=value)
            else:
                track_elem = TrackElem(Command.S, value=randomness.get_random_int(low=self.min_value, high=self.max_value))

            result.append(track_elem)

        result.append(TrackElem(command=Command.S, value=randomness.get_random_int(low=self.min_value, high=self.max_value)))
        return result

    @staticmethod
    def change_command(idx: int, track_elements: List[TrackElem], sign: str = "rnd") -> TrackElem:
        ...
        if sign == "pos":
            if (
                ...
                and COMMAND_NAME_VALUE_DICT[Command.R.name] > COMMAND_NAME_VALUE_DICT[Command.L.name]
            ):
                new_te.command = Command.R
            elif (
                new_te.command.name == Command.R.name
                and COMMAND_NAME_VALUE_DICT[Command.L.name] > COMMAND_NAME_VALUE_DICT[Command.R.name]
            ):
    ...
    def change_value(self, idx: int, track_elements: List[TrackElem], sign: str = "rnd") -> TrackElem:
        c_track_elements = copy.deepcopy(track_elements)
        track_element = c_track_elements[idx]
        new_te = TrackElem(command=track_element.command, value=track_element.value)
        max_value_dy = 2
        ...
        if new_te.command.name == Command.DY.name:
            if sign == "pos":
                if new_te.value + 0.1 > MAX_DY_VALUE:
                    ...
                    v = round(new_te.value + round(randomness.get_random_float(low=0, high=max_value_dy), 1), 1)
                    while v > MAX_DY_VALUE:
                        v = new_te.value
                        v = round(v + round(randomness.get_random_float(low=0, high=max_value_dy), 1), 1)
                    new_te.value = v
            elif sign == "neg":
                if new_te.value - 0.1 < self.min_value:
                    new_te.value = float(self.min_value)
                else:
                    v = round(new_te.value - round(randomness.get_random_float(low=0, high=max_value_dy), 2), 1)
                    while v < self.min_value:
                        v = new_te.value
                        v = round(v - round(randomness.get_random_float(low=0, high=max_value_dy), 2), 1)
                    new_te.value = v
            else:
            ...
            if sign == "pos":
                if new_te.value + self.min_value > self.max_value:
                    ...
                    v = new_te.value + randomness.get_random_int(low=self.min_value, high=max_value_int)
                    while v > self.max_value:
                        v = new_te.value
                        v = v + randomness.get_random_int(low=self.min_value, high=max_value_int)
                    new_te.value = v
            elif sign == "neg":
    ...
    def has_command_values_in_bounds(self, track_elements: List[TrackElem]) -> bool:
        for te in track_elements:
            if te.command.name == Command.L.name or te.command.name == Command.R.name or te.command.name == Command.S.name:
    ...
    def functional_constraints(self, track_elements: List[TrackElem]) -> bool:
        return (
            self.has_command_values_in_bounds(track_elements=track_elements)
            and not self.is_track_containing_loops(track_elements=track_elements)
            and not self.has_very_sharp_turns(track_elements=track_elements)
            and not self.has_overlapping_roads(track_elements=track_elements)
            and not self.has_only_straight_roads(track_elements=track_elements)
        )

    def constraints_satisfied(self, track_elements: List[TrackElem]) -> bool:
        return (
            self.functional_constraints(track_elements=track_elements)
            and self.has_difficult_curve(track_elements=track_elements, curve_angle=120)
            and self.has_at_least_curves(track_elements=track_elements, num_curves=3)
        )
    ...
    @staticmethod
    def has_length_between(track_elements: List[TrackElem], length_min: int, length_max: int) -> bool:
        length_ = 0
        for ce in track_elements:
            if ce.command.name != Command.DY.name:
                length_ += ce.value
    ...
    def has_difficult_curve(track_elements: List[TrackElem], curve_angle: int) -> bool:
        ...
        for ce in track_elements:
            ...
            if ce.command.name == Command.DY.name:
                ...
                assert previous_dy_value is not None, "Error assigning previous DY value"
                if previous_dy_value * ce.value >= curve_angle:
                    return True
    ...
    def has_very_sharp_turns(track_elements: List[TrackElem]) -> bool:
        for i in range(len(track_elements)):
            ...
            if ce.command.name == Command.L.name or ce.command.name == Command.R.name:
                ...
                assert previous_ce.command.name == Command.DY.name, "R/L command should be followed by DY. Found: {}".format(
                    previous_ce
                )
            ...
            for j in range(i + 2, len(track_points) - 1, 2):
                ...
                if (
                    ...
                    and compute_direction(
                        first_point=tp_i_1, second_point=tp_i_2, last_point=tp_j_2, last_but_one_point=tp_j_1
                    )
                    > 150.0
                ):
                    if abs(tp_i_1.x - tp_j_1.x) < TRACK_WIDTH or abs(tp_i_1.y - tp_j_1.y) < TRACK_WIDTH:
                        return True
    ...
    def road_condition(track_elements: List[TrackElem]) -> bool:
        ...
        for i in range(len(track_elements) - 1):
            ...
            if ce_1.command.name == Command.DY.name:
                road_condition = ce_2.command.name == Command.R.name or ce_2.command.name == Command.L.name
                if not road_condition:
                    return False
            elif ce_1.command.name == Command.R.name or ce_1 == Command.L.name or ce_1.command.name == Command.S.name:
                road_condition = ce_2.command.name == Command.DY.name or ce_2.command.name == Command.S.name
                if not road_condition:
                    return False
    ...
    def is_chromosome_correct(self, track_elements: List[TrackElem]) -> bool:
        ...
        return length_condition and first_command_condition and last_command_condition and road_condition
