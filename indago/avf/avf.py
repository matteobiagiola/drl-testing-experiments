...
from torch import Tensor, from_numpy, optim
...
from indago.avf.config import (
    ...
    FILTER_FAILURE_BASED_APPROACHES,
    NUM_GENERATIONS,
)
...
class Avf:
    def __init__(
        ...
        self.remove_road_constraints: bool = remove_road_constraints
        ...
        if exp_file is not None:
            ...
            assert os.path.exists(self.exp_file), "Exp file not found: {}".format(
                self.exp_file
            )
            self.failure_test_env_configs = parse_experiment_file(
                exp_file=self.exp_file, env_name=env_name
            )

        if exp_files is not None and len(exp_files) != 0:
            assert exp_file is None, "exp_file must be None"
            for exp_file in exp_files:
                ith_failure_test_env_configs = parse_experiment_file(
                    exp_file=exp_file, env_name=env_name
                )
                self.failure_test_env_configs.extend(ith_failure_test_env_configs)
            self.logger.info(
                "Replaying {} failures".format(len(self.failure_test_env_configs))
            )
            self.weights_data = np.ones(
                shape=len(
                    self.failure_test_env_configs,
                )
            )
    ...
    def load_avf_policy(
        ...
        raise NotImplementedError(
            "Train policy {} not supported".format(avf_train_policy)
        )
    ...
    def sample_test_env_configuration(
        ...
        if avf_test_policy == "prioritized_replay":
            ...
            env_config, max_prediction = self.sample_test_env_configuration_nn(
                ...
                dnn_sampling_policy=dnn_sampling_policy,
    ...
    def replay_test_failures(self) -> EnvConfiguration:
        idx_data = random.choices(
            population=np.arange(0, len(self.failure_test_env_configs))
        )[0]

        if not self.is_training and len(self.indices_data_selected) == len(
            self.failure_test_env_configs
        ):
        ...
        self.logger.info(
            "Test env configuration fail: {}".format(self.current_env_config.get_str())
        )
    ...
    def _get_env_config_for_hc(
        ...
        avf_test_policy: str,
        index_failure: int = -1,
        reuse_previous_failure_index: bool = False,
    ) -> EnvConfiguration:
        ...
        if (
            ...
            if index_failure != -1:
                if (
                    index_failure >= len(self.failed_data_items)
                    and not reuse_previous_failure_index
                ):
                    self.logger.warn(
                        "Fallback to random generation index {}/{}".format(
                            index_failure, len(self.failed_data_items)
                        )
                    )
                    ...
                    if reuse_previous_failure_index:
                        self.logger.debug(
                            "Reusing configuration with index {}".format(index_failure)
                        )
                    self.logger.debug(
                        "Index data: {}. {}/{}".format(
                            index_failure,
                            len(self.indices_data_selected),
                            len(self.failed_data_items),
                        )
                    )
                    env_config = self.failed_data_items[
                        index_failure
                    ].training_logs.get_config()
            else:
                if reuse_previous_failure_index and len(self.indices_data_selected) > 0:
                    self.logger.debug(
                        "Reusing configuration with index {}".format(
                            self.indices_data_selected[-1]
                        )
                    )
                    env_config = self.failed_data_items[
                        self.indices_data_selected[-1]
    ...
    def sample_test_env_configuration_hc(
        ...
        ), "stagnation_tolerance counter must be > 0.0. Found: {}".format(
            stagnation_tolerance
        )
        assert (
            neighborhood_size > 0
        ), "neighborhood_size counter must be > 0. Found: {}".format(neighborhood_size)
        ...
        env_config = self._get_env_config_for_hc(
            ...
            reuse_previous_failure_index=False,
        )

        hill_climbing_restart_counter = hc_counter
        max_stagnation_counter = 20
        stagnation_counter = 0
        prediction = np.inf if self.regression and self.minimize else -1.0

        current_num_predictions = 0

        initial_env_config_transformed = (
            self.preprocessed_dataset.transform_env_configuration(
                env_configuration=env_config, policy=avf_train_policy
            )
        )
        initial_env_config_prediction = (
            self.trained_avf_policy.get_failure_class_prediction(
                env_config_transformed=initial_env_config_transformed,
                dataset=self.preprocessed_dataset,
            )
        )
        self.logger.info(
            "Initial env config: {}, prediction: {}".format(
                env_config.get_str(), initial_env_config_prediction
            )
        )

        start_time = time.perf_counter()
        condition = (
            hill_climbing_restart_counter > 0
            if self.budget == -1
            else time.perf_counter() - start_time < self.budget
        )

        env_config_with_best_prediction = copy.deepcopy(env_config)
        ...
        while condition:
            ...
            if (
                avf_test_policy == "hc_saliency_rnd"
                or avf_test_policy == "hc_saliency_failure"
            ):
            ...
            neighborhood_condition = (
                ...
                if self.budget == -1
                else len(neighborhood) != neighborhood_size
                and time.perf_counter() - start_time < self.budget
            )
            while neighborhood_condition:
                if (
                    ...
                    mutated_env_config = env_config.mutate_hot(
            ...
            if len(neighborhood) != neighborhood_size:
                self.logger.warn(
                    "Not possible to generate neighbourhood for env configuration {}".format(
                        env_config.get_str()
                    )
                )
                if prediction == -1 and len(neighborhood) == 0:
                    env_config_transformed = (
                        self.preprocessed_dataset.transform_env_configuration(
                            env_configuration=env_config, policy=avf_train_policy
                        )
            ...
            for neighbor in neighborhood:
                env_configs_transformed.append(
                    self.preprocessed_dataset.transform_env_configuration(
                        env_configuration=neighbor, policy=avf_train_policy
                    )
                )
            ...
            if not stochastic:
                if self.regression and self.minimize:
                    chosen_idx = np.asarray(current_predictions).argmin()
                else:
                    chosen_idx = np.asarray(current_predictions).argmax()
            else:
                current_predictions = np.asarray(current_predictions)
                if self.regression and self.minimize:
                    indices_prediction = np.where(current_predictions >= prediction)[0]
                else:
                    indices_prediction = np.where(current_predictions <= prediction)[0]

                chosen_idx = random.choices(
                    population=indices_prediction,
                    weights=current_predictions[indices_prediction],
                )[0]

            current_prediction = current_predictions[chosen_idx]
            if self.regression and self.minimize:
                if current_prediction > prediction or math.isclose(
                    current_prediction, prediction, abs_tol=stagnation_tolerance
                ):
            ...
            env_config = neighborhood[chosen_idx]

            env_config = copy.deepcopy(env_config)

            if self.regression and self.minimize:
                if current_prediction < prediction:
                    prediction = min(prediction, current_prediction)
            else:
                if current_prediction > prediction:
                    env_config = copy.deepcopy(env_config)
                    prediction = max(prediction, current_prediction)

            env_config_with_best_prediction = copy.deepcopy(env_config)

            self.logger.debug(
                "Prediction: {}, Counter: {}, Stagnation: {}".format(
                    prediction, hill_climbing_restart_counter, stagnation_counter
                )
            )
            ...
            if stagnation_counter == max_stagnation_counter:
                ...
                if self.budget == -1:
                    ...
                    # random restart
                    hill_climbing_restart_counter = (
                        hc_counter + 1
                    )  # because there is a -1 at the end of the loop
                    stagnation_counter = 0
                    env_config = self._get_env_config_for_hc(
                        avf_test_policy=avf_test_policy,
                        index_failure=index_failure,
                        reuse_previous_failure_index=True,
                    )
        ...
        self.current_env_config = copy.deepcopy(env_config_with_best_prediction)
        if self.regression and self.minimize:
            self.logger.info(
                "Env configuration: {}; min prediction: {}".format(
                    self.current_env_config.get_str(), prediction
                )
            )
        else:
            self.logger.info(
                "Env configuration: {}; max prediction: {}".format(
                    self.current_env_config.get_str(), prediction
                )
            )
        self.predictions.append(prediction)

        return self.current_env_config, prediction
    ...
    def sample_test_env_configuration_ga(
        ...
        assert population_size > 0, "Population size must be > 0. Found: {}".format(
            population_size
        )
        assert (
            crossover_rate > 0.0
        ), "Mutation rate size must be > 0.0. Found: {}".format(crossover_rate)
        ...
        def fitness_fn(c: Chromosome) -> Tuple[float, dict]:
            ...
            env_configuration_transformed = (
                preprocessed_dataset_closure.transform_env_configuration(
        ...
        chromosome_factory = get_chromosome_factory(
            generate_env_config_fn=self.generate_random_env_configuration
        )
        ga = GeneticAlgorithm(
            population_size=population_size,
            ...
            crossover_rate=crossover_rate,
            minimize_attribution=self.minimize,
        )

        if "failure" in avf_test_policy:
            while len(ga.population) < len(self.failed_data_items):
                idx_data = random.choices(
                    population=np.arange(0, len(self.failed_data_items)),
                    weights=self.weights_data,
                )[0]
        ...
        optimal_env_configuration_transformed = (
            self.preprocessed_dataset.transform_env_configuration(
        ...
        prediction = self.trained_avf_policy.get_failure_class_prediction(
            env_config_transformed=optimal_env_configuration_transformed,
            dataset=self.preprocessed_dataset,
        )
    ...
    def sample_test_env_configuration_nn(
        ...
        if dnn_sampling_policy == "original":
            ...
            if len(self.all_configurations) == 0:
                ...
                if self.regression and self.minimize:
                    prediction_idx = np.argmin(predictions)
                else:
                    prediction_idx = np.argmax(predictions)

                env_config = env_configurations[prediction_idx]
            ...
            env_config, prediction = self.all_configurations.pop(0)
            self.current_env_config = env_config
            self.logger.info(
                "Env configuration: {}; prediction: {}".format(
                    self.current_env_config.get_str(), prediction
                )
            )
            self.predictions.append(prediction)
        ...
        raise NotImplementedError(
            "Unknown dnn sampling policy: {}".format(dnn_sampling_policy)
        )
    ...
    def generate_random_env_configuration(self) -> EnvConfiguration:
        if self.env_name == PARK_ENV_NAME:
            ...
            env_config = DonkeyEnvConfiguration(
                remove_road_constraints=self.remove_road_constraints
            ).generate_configuration()
        elif self.env_name == CARTPOLE_ENV_NAME:
            env_config = CartPoleEnvConfiguration().generate_configuration()
        else:
            raise NotImplementedError(
                "Environment {} not supported".format(self.env_name)
            )
    ...
    def generate_env_configuration(self) -> EnvConfiguration:
        ...
        if not self.is_training:
            ...
            if (
                ...
                and self.avf_test_policy != "replay_test_failure"
            ):
                if self.preprocessed_dataset is None:
                    ...
                    if self.avf_test_policy in AVF_TEST_POLICIES_WITH_DNN:
                        ...
                        if os.path.exists(output_scaler_path):
                            self.logger.info(
                                "Loading output scaler: {}".format(output_scaler_path)
                            )
                            self.preprocessed_dataset.output_scaler = load(
                                output_scaler_path
                            )
                        if os.path.exists(input_scaler_path):
                            self.logger.info(
                                "Loading input scaler: {}".format(input_scaler_path)
                            )
                            self.preprocessed_dataset.input_scaler = load(
                                input_scaler_path
                            )

                if (
                    self.avf_test_policy != "prioritized_replay"
                    and self.trained_avf_policy is None
                ):
            ...
            if len(self.failure_probability_env_configurations) == 0:
                ...
                if (
                    ...
                    num_failures_dataset = sum(
                        [
                            data.label
                            for data in self.preprocessed_dataset.get()
                            if data.label == 1
                        ]
                    )
                    if (
                        ...
                        raise NotImplementedError(
                            "Parallel mode not supported for {} and {} episodes since they are less than the number of failures {}".format(
                                ...
                                self.num_episodes,
                                num_failures_dataset,
                            )
                        )

                    with Parallel(
                        n_jobs=-1, batch_size="auto", backend="loky"
                    ) as parallel:
                        res = parallel(
                            (
                    ...
                    for env_config, time_elapsed, max_prediction in res:
                        self.logger.info(
                            "Env configuration: {}; max prediction: {}".format(
                                env_config.get_str(), max_prediction
                            )
                        )
                        self.logger.info(
                            "Times elapsed: {} s".format(round(time_elapsed, 2))
                        )
                        times_elapsed.append(time_elapsed)
                        for _ in range(self.num_runs_each_env_config):
                            self.failure_probability_env_configurations.append(
                                env_config
                            )
                else:
                    for i in range(self.num_episodes):
    ...
    def store_testing_logs(self, training_logs: TrainingLogs) -> None:
        if not self.is_training:
            assert (
                self.testing_strategy_name is not None
            ), "Testing strategy name is not assigned"
            assert self.num_episodes != -1, "Num episodes not assigned"
            if self.num_trials != -1:
                filepath = os.path.join(
                    self.log_path,
                    self.testing_strategy_name,
    ...
    def store_training_logs(self, training_logs: TrainingLogs) -> None:
        if self.storing_logs:
            ...
            log_path = os.path.join(
                self.log_path,
                "avf_log_{}_{}.json".format(self.file_num_first, file_num_last),
            )
    ...
    @staticmethod
    def get_bins_mse_loss(
        policy: AvfPolicy, data: Tensor, labels: Tensor, bins_count: List[int]
    ) -> float:
        assert (
            min(labels) >= 0.0 and max(labels) <= 1.0
        ), "Labels not supported for this loss. All values should be in [0, 1]."
        binned_labels = TorchDataset.get_bin_indexes(
            labels=to_numpy(tensor=labels), bin_edges=TorchDataset.get_bin_edges()
        )
        ...
        if (
            len(
                list(
                    filter(
                        lambda value: value == 1, list(Counter(binned_labels).values())
                    )
                )
            )
            > 0
            ...
            binned_labels = TorchDataset.get_custom_bin_indexes(
                labels=to_numpy(tensor=labels)
            )

        data_bins, labels_bins = [], []
        for bin_count in bins_count:
            if bin_count < 0:
                bin_count = list(range(min(binned_labels), max(binned_labels) + 1))[
                    bin_count
                ]

            if bin_count == 0 and bin_count not in binned_labels:
                bin_count = min(binned_labels)

            if bin_count == 1 and bin_count not in binned_labels:
                bin_count = min(binned_labels) + 1

            if bin_count not in binned_labels:
                continue

            try:
                data_bin, labels_bin = Avf.get_data_and_labels_for_bin(
                    binned_labels=binned_labels,
                    data=to_numpy(tensor=data),
                    labels=to_numpy(tensor=labels),
                    bin_num=bin_count,
                )
            ...
            labels_bins.append(labels_bin)

        if len(data_bins) == 0:
            return math.inf

        data_all, labels_all = from_numpy(np.concatenate(data_bins)), from_numpy(
            np.concatenate(labels_bins)
        )

        if len(data_all) == 1:
            # the mean_squared_error function throws a TypeError: Singleton array cannot be considered a valid collection
            # when the length of the predictions is 1.
            return math.inf

        predictions = policy.forward_and_loss(
            data=data_all, target=labels_all, training=False
        )

        return mean_squared_error(
            y_true=to_numpy(tensor=labels_all), y_pred=to_numpy(tensor=predictions)
        )
    ...
    def train_dnn(
        ...
        if (
            train_dataset_ is not None
            and validation_dataset_ is not None
            and test_dataset_ is not None
        ):
            ...
            self.logger.info(
                "Train data labels proportion: {}".format(train_dataset.labels.mean())
            )
            self.logger.info(
                "Validation data labels proportion: {}".format(
                    validation_dataset.labels.mean()
                )
            )
            self.logger.info(
                "Test data labels proportion: {}".format(test_dataset.labels.mean())
            )

            if not self.regression:
                failures_train_dataset = reduce(
                    lambda a, b: a + b,
                    filter(lambda label: label == 1, train_dataset.labels),
                )
                ...
                non_failures_train_dataset = (
                    len(train_dataset.labels) - failures_train_dataset
                )
                non_failures_validation_dataset = (
                    len(validation_dataset.labels) - failures_validation_dataset
                )
                non_failures_test_dataset = (
                    len(test_dataset.labels) - failures_test_dataset
                )

                self.logger.info(
                    "Failures train dataset: {}/{} non failures".format(
                        failures_train_dataset, non_failures_train_dataset
                    )
                )
                self.logger.info(
                ...
                self.logger.info(
                    "Failures test dataset: {}/{} non failures".format(
                        failures_test_dataset, non_failures_test_dataset
                    )
                )
            ...
            if len(train_dataset_.data) < batch_size:
                ...
                batch_size = list(
                    filter(lambda n: n < len(train_dataset_.data), powers_of_two)
                )[-1]
                self.logger.info(
                    "New batch size for training dataset: {}".format(batch_size)
                )

            if len(validation_dataset_.data) < batch_size:
                # find first power of two smaller than validation_set length
                batch_size = list(
                    filter(lambda n: n < len(validation_dataset_.data), powers_of_two)
                )[-1]
                self.logger.info(
                    "New batch size for validation dataset: {}".format(batch_size)
                )
        else:
            ...
            if not no_test_set and len(test_validation_dataset.data) > 0:
                ...
                validation_data, validation_labels = (
                    test_validation_dataset.data,
                    test_validation_dataset.labels,
                )
            ...
            if 0 < len(validation_data) < batch_size:
                # find first power of two smaller than validation_set length
                batch_size = list(
                    filter(lambda n: n < len(validation_data), powers_of_two)
                )[-1]
                self.logger.info(
                    "New batch size for validation dataset: {}".format(batch_size)
                )
            ...
            if not no_test_set:
                self.logger.info(
                    "Test data labels proportion: {}".format(test_labels.mean())
                )
        ...
        optimizer = optim.Adam(
            params=avf_policy.parameters(), lr=learning_rate, weight_decay=weight_decay
        )
            ...
            if (epoch == 0 or epoch % 5 == 0) and len(validation_dataset.data) > 0:
                ...
                with torch.no_grad():
                    ...
                    with tqdm(validation_dataloader, unit="batch") as validation_epoch:
                        ...
                        for (
                            ...
                            if predictions is not None:
                                if self.regression:
                                    val_accuracy += r2_score(
                                        y_true=to_numpy(validation_target),
                                        y_pred=to_numpy(predictions),
                                    )
                ...
                if not self.regression:
                    if precision > best_val_precision:
                        ...
                        if (
                            train_dataset_ is None
                            and validation_dataset_ is None
                            and test_dataset_ is None
                        ) or save_model:
                    ...
                    if val_loss < best_val_loss:
                        ...
                        self.logger.info(
                            "Corresponding R2 score: {}".format(val_accuracy)
                        )
                        self.logger.info(
                            "Corresponding validation mae: {}".format(best_val_mae)
                        )
                        if (
                            train_dataset_ is None
                            and validation_dataset_ is None
        ...
        if len(test_dataset.data) > 0 or (
            no_test_set and len(validation_dataset.data) > 0
        ):
            ...
            avf_policy.load(filepath=save_path)
            ...
            with torch.no_grad():
                ...
                test_loss, _ = avf_policy.forward_and_loss(
                    data=test_data, target=test_target, weights=weights, training=True
                )
                test_loss = test_loss.item()

                if self.regression:
                    predictions = avf_policy.forward_and_loss(
                        data=test_data,
                        target=test_target,
                        training=False,
                    )
                    ...
                    test_mse = mean_squared_error(
                        y_true=np_target, y_pred=np_predictions
                    )
                    ...
                    self.logger.info(
                        "R2 score on test set: {:.2f}".format(test_accuracy)
                    )
                    self.logger.info("MAE score on test set: {:.2f}".format(test_mae))
                    self.logger.info("MSE score on test set: {:.2f}".format(test_mse))

                    regression_values_between_zero_and_one = bool(
                        min(test_target) >= 0.0 and max(test_target) <= 1.0
                    )
                    if regression_values_between_zero_and_one:

                        if dataset.output_scaler is not None:
                            self.logger.info(
                                "Unnormalizing labels to compute bins mse loss"
                            )
                            # values have been normalized between 0 and 1
                            # in reshaping I'm assuming there is only one feature
                            test_target_array = dataset.output_scaler.inverse_transform(
                                X=to_numpy(tensor=test_target).reshape(-1, 1)
                            )
                            test_target = from_numpy(test_target_array)

                        binned_labels = TorchDataset.get_bin_indexes(
                            labels=to_numpy(tensor=test_target),
                            bin_edges=TorchDataset.get_bin_edges(),
                        )
                        ...
                        if (
                            len(
                                list(
                                    filter(
                                        ...
                                        list(Counter(binned_labels).values()),
                                    )
                                )
                            )
                            > 0
                            ...
                            self.logger.warn(
                                f"There is at least one bin with one sample only. Falling back to two bins: "
                                f"{Counter(binned_labels)}"
                            )

                        if len(Counter(binned_labels)) > 3:
                            bins_count_first = [0, 1]
                            bins_count_last = [-2, -1]
                        else:
                            bins_count_first = [0]
                            bins_count_last = [-1]

                        test_mse_first_bins = self.get_bins_mse_loss(
                            bins_count=bins_count_first,
                            policy=avf_policy,
                            data=test_data,
                            labels=test_target,
                        )

                        test_mse_last_bins = self.get_bins_mse_loss(
                            bins_count=bins_count_last,
                            policy=avf_policy,
                            data=test_data,
                            labels=test_target,
                        )

                        self.logger.info(f"MSE score first bins: {test_mse_first_bins}")
                        self.logger.info(f"MSE score last bins: {test_mse_last_bins}")

                else:
                    logits, predictions = avf_policy.forward_and_loss(
                        data=test_data,
                        target=test_target,
                        training=False,
                    )
                    ...
                    self.logger.info(
                        "Accuracy test set: {}".format(correct / len(test_target))
                    )
                    ...
                    if (
                        ...
                        if not no_test_set:
                            ...
                            plt.savefig(
                                os.path.join(
                                    self.log_path,
                                    "avf-{}-{}-{}-roc-auc-validation.png".format(
                                        avf_train_policy,
                                        training_progress_filter,
                                        oversample_minority_class_percentage,
                                    ),
        ...
        if (
            ...
            if dataset.output_scaler is not None:
                ...
                self.logger.info(
                    "Saving output scaler in {}".format(output_scaler_path)
                )
    ...
    @staticmethod
    def get_data_and_labels_for_bin(
        binned_labels: np.ndarray, data: np.ndarray, labels: np.ndarray, bin_num: int
    ) -> Tuple[np.ndarray, np.ndarray]:
        assert (
            bin_num in binned_labels
        ), f"Bin num {bin_num} not in binned labels {binned_labels}"
        indices_bin_zero = np.where(binned_labels == bin_num)[0]
        if len(indices_bin_zero) == 0:
            raise ValueError(f"No samples in bin {indices_bin_zero}.")
        data_bin = data[indices_bin_zero]
        labels_bin = Dataset.get_regression_labels_from_split(
            split_data=data_bin, whole_data=data, continuous_labels=labels
        )
        return data_bin, labels_bin
        ...
        if avf_train_policy in AVF_DNN_POLICIES:
            if training_progress_filter is not None:
                ...
                save_path = os.path.join(
                    self.log_path,
                    "best-avf-{}-{}-{}.pkl".format(
                        avf_train_policy, oversample_minority_class_percentage, layers
                    ),
                )
