...
if __name__ == "__main__":
    ...
    logging.basicConfig(filename=os.path.join(log_path, "{}.txt".format(log_filename)), filemode="w", level=logging.DEBUG)
    ...
    if args.build_heldout_test:
        ...
        if args.heldout_test_file is not None:
            assert os.path.exists(os.path.join(log_path, args.heldout_test_file)), "Heldout test file {}".format(
            ...
            assert os.path.exists(
                os.path.join(log_path, args.online_dataset_train)
            ), "Online dataset train file {} does not exist".format(os.path.join(log_path, args.online_dataset_train))

            assert os.path.exists(
                os.path.join(log_path, args.online_dataset_train)
            ), "Online dataset test file {} does not exist".format(os.path.join(log_path, args.online_dataset_test))

            numpy_train_dict = np.load(os.path.join(log_path, args.online_dataset_train))
            train_validation_data = numpy_train_dict["train_data"]
            train_validation_labels = numpy_train_dict["train_labels"]

            numpy_test_dict = np.load(os.path.join(log_path, args.online_dataset_test))
