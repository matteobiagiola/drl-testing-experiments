...
class Chromosome:
    def __init__(self, env_config: EnvConfiguration, minimize: bool = False):
        self.fitness = None
    ...
    def mutate_hot(
        ...
        new_env_config = self.env_config.mutate_hot(
            attributions=attributions, mapping=mapping, minimize=minimize
        )
        if new_env_config is not None:
            return Chromosome(env_config=new_env_config, minimize=minimize)
        return None

    def crossover(
        self, c: "Chromosome", pos1: int, pos2: int
    ) -> Optional["Chromosome"]:
        new_env_config = self.env_config.crossover(
            other_env_config=c.env_config, pos1=pos1, pos2=pos2
        )
        if new_env_config is not None:
            return Chromosome(env_config=new_env_config)
        return None
