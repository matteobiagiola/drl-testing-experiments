...
def compute_coverage_entropy(
    ...
    folder_name = os.path.join(folder, "input_diversity") if tp == "input" else os.path.join(folder, "output_diversity")
    ...
    if args.pattern:
        for idx in range(len(args.names)):
            ...
            if name == "random" or "prioritized_replay" in name:
                directories = glob.glob(os.path.join(args.folder, "replay_test_failure_{}-*-trial".format(name)))
            else:
                directories = glob.glob(
                    os.path.join(args.folder, "replay_test_failure_{}-{}-*-trial".format(args.avf_train_policy, name))
                )
